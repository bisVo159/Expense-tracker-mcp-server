...
@mcp.tool()
def edit_expense(subcategory, date, amount=None, category=None, note=None):
...
@mcp.tool()
def delete_expense(date,subcategory):
    '''Delete an expense entry by its date and subcategory.'''
    with sqlite3.connect(DB_PATH) as c:
        cur = c.execute("DELETE FROM expenses WHERE date = ? and subcategory = ?", (date,subcategory))
        return {"status": "ok", "rows_affected": cur.rowcount}
